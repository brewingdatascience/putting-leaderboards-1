...
* Why collect or analyze disc putting data?
    * This is a demonstration of reproducible research and transparency in the process of tabulating statistics from raw game data. Each approved putting course is a set of 18 well-defined shots, typically comprised of three circle2 putts and 15 circle1 putts. With unlimited numbers of players putting the exact same holes -and using the same [data entry form](https://docs.google.com/forms/d/e/1FAIpQLSdZlQnmdbSv2BY1mxqzvmxMnkSR56L9buXUHy6r0ych_hm2vg/viewform)- high-quality data and patterns emerge, providing unique insights into disc putting.  
...
a <- player_circle1 %>% 
  ...
  head(10) %>% 
  select(Player = player, 'Circle1 average'= circle1mean, 'Circle1 attempts'=circle1attempts, 'Circle1 putts made' = circle1made) 
...
a <- player_circle2 %>% 
  ...
  head(10) %>% 
  select(Player = player, 'Circle2 average'= circle2mean, 'Circle2 attempts'=circle2attempts, 'Circle2 putts made' = circle2made)
...
playerHoleSummary <- tidyPutts %>%
  ...
  ungroup() %>% 
  select(Player = player, 'Hole average' = x, Hole, 'Attempts' = putts.made_n,  'Made' = putts.made_sum) 
...
[Course Registration](https://docs.google.com/forms/d/e/1FAIpQLSfcKdf9TDGmvH833hY7VjvGoKKUQCgsLKIQF-A2UbsIelfTSA/viewform?usp=sf_link)
...
`r max(tidyPutts$Timestamp)` >> Most recent game in dataset
